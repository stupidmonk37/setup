...
# ========================================
# PDU Power Off Script
# ========================================
# This script takes a hostname (like c0r21-gn1) and turns off all 4 PSU connections
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --ping                Monitor node with ping to verify shutdown"
            ...
            echo "  3. Turn OFF all 4 connections using check-outlet.sh"
            echo "  4. If --ping is specified, monitor the full FQDN until it goes offline"
            exit 0
            ;;
        -*)
...
# Function to monitor node shutdown with ping
monitor_shutdown() {
    local fqdn="$1"
    local timeout="$2"
    
    echo "========================================="
    echo "Monitoring node shutdown: $fqdn"
    ...
    if ! ping -c 1 -W 5 "$fqdn" &>/dev/null; then
        echo "⚠️  Node $fqdn is already unreachable"
        return 0
    fi
    
    echo "✓ Node $fqdn is currently reachable"
    echo "Monitoring for shutdown... (Press Ctrl+C to stop monitoring)"
    echo
    
    local start_time=$(date +%s)
    local last_response_time=$start_time
    ...
    while true; do
        ...
        if [[ $elapsed -gt $timeout ]]; then
            echo
            echo "⏰ Timeout reached (${timeout} seconds). Node may still be online."
        ...
        if ping -c 1 -W 3 "$fqdn" &>/dev/null; then
            ...
            printf "\r[%s] Node still online (elapsed: %ds)" "$(date '+%H:%M:%S')" "$elapsed"
        else
            # Node didn't respond
            consecutive_failures=$((consecutive_failures + 1))
            printf "\r[%s] No response (%d/%d) (elapsed: %ds)" "$(date '+%H:%M:%S')" "$consecutive_failures" "3" "$elapsed"
            
            # Consider node offline after 3 consecutive failures
            if [[ $consecutive_failures -ge 3 ]]; then
                echo
                echo
                echo "🔌 Node $fqdn has gone OFFLINE!"
                local offline_time=$((current_time - last_response_time))
                echo "Time from last response to offline detection: ${offline_time} seconds"
                echo "Total monitoring time: ${elapsed} seconds"
                return 0
            fi
        fi
...
while IFS='|' read -r device pdu_num port_num cable; do
    ...
    cmd="./check-outlet.sh --cluster=$cluster --rack=$rack --pdu=$pdu_num --port=$port_num --state=off"
    ...
    if [[ -n "$dry_run" ]]; then
        ...
        if output=$($cmd 2>&1); then
            if [[ -n "$verbose" ]]; then
                ...
                echo "  ✓ Successfully turned OFF $device on PDU$pdu_num/Port$port_num"
            fi
            success_count=$((success_count + 1))
        else
            echo "  ✗ Failed to turn OFF $device on PDU$pdu_num/Port$port_num"
            if [[ -n "$verbose" ]]; then
...
if [[ "$failure_count" -gt 0 ]]; then
    ...
    if [[ -n "$dry_run" ]]; then
        ...
        echo "✅ All PSU connections for $hostname have been turned OFF"
        
        # Start ping monitoring if requested
        if [[ -n "$ping_monitor" ]]; then
            echo
            fqdn=$(construct_fqdn "$hostname" "$cluster")
            echo "Starting ping monitoring to verify node shutdown..."
            
            if monitor_shutdown "$fqdn" "$ping_timeout"; then
                echo "✅ Node shutdown confirmed via ping monitoring"
            else
                echo "⚠️  Node shutdown monitoring timed out or failed"
                echo "The PSU connections have been turned off, but the node may still be online"
                echo "This could indicate:"
                echo "  - Node has backup power or UPS"
                echo "  - Network connectivity issues"
                echo "  - Node is in a different power state"
            fi
        fi
    fi
fi 