...
from utils import colorize, display_crossrack_table, is_xrk_name, kubectl_get_json_validation


def valid_rack_name(value: str) -> str:
    if not is_xrk_name(value):
        raise argparse.ArgumentTypeError(
            f"Invalid rack format: '{value}'. Must match c<digit>r<digits>-c<digit>r<digits> (e.g., c0r1-c0r2)"
        )
    return value
