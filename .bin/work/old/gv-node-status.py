    ...
    def generate_validation_table(cluster_id_prefix):
        ...
        for test, node_data in merged_validations.items():
            ...
            for short_node in short_node_names:
                ...
                padded = f"{status:^9}"
                if status == "Success":
                    colorized = f"{GREEN}{padded}{RESET}" if USE_COLOR else padded
                else:
                    colorized = f"{RED}{padded}{RESET}" if USE_COLOR else padded
...
def run_with_temp_venv():
    ...
    try:
        ...
        if os.path.exists("requirements.txt"):
            ...
            print("📦 Installing pyyaml and colorama manually...")
            subprocess.check_call([pip, "install", "pyyaml", "colorama"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        subprocess.check_call([python, script_path, *sys.argv[1:]])
...
def install_dependencies():
    try:
        ...
        # Optionally write requirements.txt if missing
        if not os.path.exists("requirements.txt"):
            with open("requirements.txt", "w") as req:
                req.write("pyyaml\ncolorama\n")
